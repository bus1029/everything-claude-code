...
Create or update codemaps in `<CODEMAP_DIR>/` (or `<REPORTS_DIR>/codemaps/`):
...
## Key Files
<APP_DIR>/services/user.ts (business logic, 120 lines)
<APP_DIR>/repos/user.ts (database access, 80 lines)
...
Write a summary to `<REPORTS_DIR>/codemap-diff.txt`:
...
- `<CODEMAP_DIR>`는 코드맵 문서 루트 디렉터리로 치환한다. 예: `docs/CODEMAPS`, `documentation/codemaps`
- `<REPORTS_DIR>`는 분석/리포트 산출물 디렉터리로 치환한다. 예: `.reports`, `reports`
- `<APP_DIR>`는 프로젝트의 주요 소스 루트 디렉터리로 치환한다. 예: `src`, `app`, `lib`, `packages/<name>/src`
