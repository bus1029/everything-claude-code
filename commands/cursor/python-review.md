...
# Testing
pytest --cov=<PY_APP_PKG> --cov-report=term-missing
```
...
## Files Reviewed
- <PY_APP_DIR>/routes/user.py (modified)
- <PY_APP_DIR>/services/auth.py (modified)
...
File: <PY_APP_DIR>/routes/user.py:42
...
File: <PY_APP_DIR>/services/auth.py:18
...
File: <PY_APP_DIR>/services/auth.py:25
...
File: <PY_APP_DIR>/routes/user.py:55
...
## Formatting Required
Run: `black <PY_APP_DIR>/routes/user.py <PY_APP_DIR>/services/auth.py`
```

## Notes

- `<PY_APP_DIR>`는 Python 애플리케이션/패키지 루트 디렉터리로 치환한다. 예: `src`, `app`, `your_package`
- `<PY_APP_PKG>`는 pytest 커버리지 기준 패키지/모듈 이름으로 치환한다. 예: `src`, `app`, `your_package`
