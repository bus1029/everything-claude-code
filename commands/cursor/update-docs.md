...
Generate or update `<DOCS_DIR>/CONTRIBUTING.md` with:
...
Generate or update `<DOCS_DIR>/RUNBOOK.md` with:
...
──────────────────────────────
Updated:  <DOCS_DIR>/CONTRIBUTING.md (scripts table)
Updated:  <DOCS_DIR>/ENV.md (3 new variables)
Flagged:  <DOCS_DIR>/DEPLOY.md (142 days stale)
Skipped:  <DOCS_DIR>/API.md (no changes detected)
──────────────────────────────
```
...
- `<DOCS_DIR>`는 문서 루트 디렉터리로 치환한다. 예: `docs`, `documentation`, `.docs`
