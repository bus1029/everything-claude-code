...
Look up up-to-date documentation for a library, framework, or API and return a summarized answer with relevant code snippets. Use Context7 MCP tools so the answer reflects current docs rather than stale assumptions.
...
Return a short, accurate answer backed by current docs, plus any code snippets that help. If Context7 is not available, say so and answer with best-effort model knowledge, noting that the result may be outdated.
