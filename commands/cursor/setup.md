...
- **코드 기준 읽기 전용**: `/setup` 단계에서는 애플리케이션 코드/설정(동작에 영향을 주는 파일)을 수정/생성하지 않는다.
  - 단, **산출물 문서**(`<CONTEXT_PACK_PATH_PRIMARY>` 또는 `<CONTEXT_PACK_PATH_FALLBACK>`)의 생성/갱신은 허용한다.
...
- 권장 저장 위치: `<CONTEXT_PACK_PATH_PRIMARY>` (없으면 생성)
- 대안: `<CONTEXT_PACK_PATH_FALLBACK>`

> 플레이스홀더 권장값:
> - `<CONTEXT_PACK_PATH_PRIMARY>`: `docs/PROJECT_CONTEXT.md` (문서 디렉터리가 없으면 프로젝트 관례에 맞게 변경)
> - `<CONTEXT_PACK_PATH_FALLBACK>`: `.cursor/PROJECT_CONTEXT.md`
...
- 컨텍스트 팩은 정확도보다 재현성/유지보수성을 우선하고, 필요 시 단계적으로 확장한다.
