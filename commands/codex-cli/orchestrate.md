...
- `explorer`로 현재 구조와 진입점을 조사
- callable하다면 `code-architect`로 구현 청사진 작성
- callable하다면 `tdd-guide`로 테스트 분해
- 구현 후 `reviewer` 검토
- 보안 민감 경로면 callable할 때 `security-reviewer` 추가

### Bugfix

- `explorer`로 재현 경로와 영향 범위 확인
- callable하다면 `tdd-guide`로 재현 테스트 작성
- 수정 후 `reviewer` 검토

### Refactor

- callable하다면 `architect` 또는 `code-architect`로 안전한 변경 범위 정의
