...
#
# When CWD_SKILLS_DIR is omitted, defaults to $PWD/.cursor/skills so the
# script always picks up project-level skills without relying on the caller.
#
# Environment:
#   SKILL_STOCKTAKE_GLOBAL_DIR   Override ~/.cursor/skills (for testing only;
#                                do not set in production — intended for bats tests)
#   SKILL_STOCKTAKE_PROJECT_DIR  Override project dir detection (for testing only)

set -euo pipefail

GLOBAL_DIR="${SKILL_STOCKTAKE_GLOBAL_DIR:-$HOME/.cursor/skills}"
CWD_SKILLS_DIR="${SKILL_STOCKTAKE_PROJECT_DIR:-${1:-$PWD/.cursor/skills}}"
...
OBSERVATIONS="${SKILL_STOCKTAKE_OBSERVATIONS:-$HOME/.cursor/observations.jsonl}"

# Validate CWD_SKILLS_DIR looks like a .cursor/skills path (defense-in-depth).
# Only warn when the path exists — a nonexistent path poses no traversal risk.
if [[ -n "$CWD_SKILLS_DIR" && -d "$CWD_SKILLS_DIR" && "$CWD_SKILLS_DIR" != */.cursor/skills* ]]; then
  echo "Warning: CWD_SKILLS_DIR does not look like a .cursor/skills path: $CWD_SKILLS_DIR" >&2
fi
